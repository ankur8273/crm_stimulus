...
class LeadController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // dd(auth('employee')->user()->permissions,permission(auth('employee')->user(),'Lead'));
        $name = null;
        $email = null;
        $phone = null;
        $user_id = null;
        ...
        if(has_permission('Lead')){
            ...
            $leads = Lead::where('user_id',auth('employee')->user()->id)->orderBy('id','desc');
        }
        ...
        return view('employee.leads.index',compact('leads','name','email','phone','user_id','lead_status','date','start','end'));
    }
    ...
    public function details(Request $request,$id)
    {
        ...
        // dd($dates);
        return view('employee.leads.details',compact('lead','previous','next','total','leadnotes','callnotes','documents','activitieDates','projects'));
    }
    ...
    public function change_leadStatus(Request $request,$id)
    {
        ...
        $this->send_notification([
            'lead'=> $lead,
            'message'=> 'New Status is '.$request->status.'Lead Status Changed By '
        ]);
    ...
    {
        $lead = Lead::findOrFail($id);
        $lead->next_fellowup_remark = $request->comment;
        $lead->next_followup = $request->fellowup;
        $lead->next_action_date = $request->fellowup_date;
        $lead->save();
        $this->send_notification([
            'lead'=> $lead,
            'message'=> 'Lead Updated for next fellowup By '
        ]);
    ...
    {
        $lead = Lead::findOrFail($id);
        $lead->add_to_favourite = $lead->add_to_favourite==1?0:1;
        $lead->save();
        $this->send_notification([
            'lead'=> $lead,
            'message'=> 'Lead added to favourite By '
        ]);
        
        return $lead->add_to_favourite==1?1:2;
    }
    ...
    public function add_note(Request $request)
    {
        ...
        $leadnote->user_id = auth('employee')->user()->id;
    ...
    public function import(Request $request)
    {
        if($request->has('file')){
            $file = $request->file('file');
            Excel::import(new LeadsImport('employee'), $file);
            return redirect()->back()->with('success', 'File Uploded Successfully!');
        }
        $this->send_notification([
            'lead'=> null,
            'message'=> 'Some New leads added By '
        ]);
        ...
        foreach($users as $user){
            ...
            if(permission($user,'Lead') || ($data['lead'] && $data['lead']->user_id==$user->id)){
                $user->notify(new UserNotification([
                    'lead_id'=> $data['lead']?$data['lead']->id:null,
