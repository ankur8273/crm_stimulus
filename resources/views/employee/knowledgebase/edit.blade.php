...
@section('content')
         ...
         <div class="page-wrapper">
            <div class="content container-fluid pb-0">
               ...
               <div class="row">
                  <form action="{{route('employee.knowledgebase.update',$knowledgebase->id)}}" method="post" enctype="multipart/form-data">
                   @csrf
                   <div class="row">
                      <div class="col-sm-6">
                         <div class="input-block mb-3">
                            <label class="col-form-label">Title</label>
                            <input class="form-control" type="text" name="title" value="{{$knowledgebase->title}}" required>
                         </div>
                      </div>
                      <div class="col-sm-6">
                         <div class="input-block mb-3">
                            ...
                            <select class="select" name="category" required>
                            	@foreach($categories as $key=>$category)
                               		<option {{$knowledgebase->category == $category->name?'selected':''}}>{{$category->name}}</option>
                               	@endforeach
                            </select>
                         </div>
                      </div>
                   ...
                   <div class="row">
                      <div class="col-sm-12">
                         <div class="input-block mb-3">
                            ...
                            <textarea class="form-control summernote" name="details" required>{{$knowledgebase->details}}</textarea>
                         </div>
                      </div>
                   </div>
