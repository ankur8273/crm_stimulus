			...
			<div class="row align-items-center">
				...
				<div class="col-auto float-end ms-auto">
					...
					<div class="view-icons">
						<a href="{{route('employee.channel-partner.list')}}" class="grid-view btn btn-link"><i
						...
						<tbody>
							@foreach($CPartners as $CPartner)
								<tr>
									<td>
										<a
											href="{{route('employee.channel-partner.details', $CPartner->id)}}">{{$CPartner->cp_name}}</a>
									</td>
									...
									<td>
										<div class="dropdown action-label">
											<a href="#" class="btn btn-white btn-sm btn-rounded dropdown-toggle"
												...
												{{$CPartner->status == 1 ? 'Active' : 'Inactive' }}</a>
											<div class="dropdown-menu">
												<a class="dropdown-item"
													href="{{route('employee.channel-partner.change_status', $CPartner->id)}}?status=1"><i
														class="fa-regular fa-circle-dot text-success"></i> Active</a>
												<a class="dropdown-item"
													href="{{route('employee.channel-partner.change_status', $CPartner->id)}}?status=0"><i
									...
									<td class="text-end">
										<div class="dropdown dropdown-action">
											...
											<div class="dropdown-menu dropdown-menu-right">
											   @if(has_permission('Write-CP'))
												<a class="dropdown-item" href="javascript:void(0);"
													data-data="{{json_encode($CPartner->toArray())}}"
													data-href="{{route('employee.channel-partner.update', $CPartner->id)}}"
													onclick="edit_Cpartner($(this))"><i
														class="fa-solid fa-pencil m-r-5"></i>
													Edit</a>
												@endif
												@if(has_permission('Read-CP'))
												<a class="dropdown-item"
													href="{{route('employee.channel-partner.details', $CPartner->id)}}"><i
														class="fa-regular fa-eye"></i> Preview</a>
												@endif
												@if(has_permission('Delete-CP'))
												<a class="dropdown-item" href="javascript:void(0);"
													data-href="{{route('employee.channel-partner.delete', $CPartner->id)}}"
...
<script type="text/javascript">
	...
	function edit_Cpartner(el) {
		...
		// ('#edit_branch_id').val(data.looking_for).change();
...
<script>
	$(document).ready(function () {
		$('#createForm').on('submit', function (e) {
			...
			$.ajax({
				url: "{{route('employee.channel-partner.store')}}",  // Replace with your route
				type: "POST",
				data: formData,
				contentType: false,
				processData: false,
		...
		$('#editForm').on('submit', function (e) {
			...
			$.ajax({
				url: "{{route('employee.channel-partner.update')}}",  // Replace with your route
				type: "POST",
				data: formData,
				contentType: false,
				processData: false,
